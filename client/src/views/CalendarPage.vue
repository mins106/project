<template>
    <div class="calendar-page">
        ...
        <div class="calendar-nav">
            <div class="select-group">
                <select v-model="selectedYear" @change="onYearChange">
                    <option v-for="year in Object.keys(allowedMonths)" :key="year" :value="Number(year)">
                        {{ year }}
                    </option>
                </select>
                <span>년</span>
        ...
        <table class="calendar-table">
            <thead>
                <tr>
                    <th v-for="(day, idx) in days" :key="day" :class="{ sun: idx === 6, sat: idx === 5 }">{{ day }}</th>
                </tr>
            </thead>
            <tbody>
                <tr v-for="(week, i) in calendar" :key="i">
                    <td v-for="(cell, j) in week" :key="j"
                        :class="{ sun: j === 6, sat: j === 5, today: isToday(cell) }">
                        <div v-if="cell !== null" class="day-card">
                            <div class="day-number">{{ cell }}</div>
                            <div v-for="event in getEvents(cell)" :key="event" class="event-text">{{ event }}</div>
                        </div>
                    </td>
                </tr>
            </tbody>
